...
Unified SASS utility ruleset for layout, positioning, spacing, typography, borders and basic animations. Every utility ships with mobile and tablet responsive variants out of the box.
...
Breakpoints are defined in `_variables.sass` (`$mobile`, `$tablet`, `$desktop`) and can be overridden by re-declaring them before importing the package.
...
For backwards compatibility every spacing class also has a short alias without the unit suffix. The unit is inferred from the order it's generated (rem → px → em with later scales overriding earlier ones).
...
```sass
@use "@vcalderondev/sass-ruleset/src/variables" with (
  $mobile: 640px,
  $tablet: 1024px
)
@use "@vcalderondev/sass-ruleset"
```
